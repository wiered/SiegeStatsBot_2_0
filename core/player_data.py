...
class PastSeasonRankedRecords:
    ...
    class Season:
        def __init__(self, season) -> None:
            ...
            self.mmr_point: str = (
                "ᐃ" if self.mmr_change > 0 else "ᐁ" if self.mmr_change < 0 else "ᐅ"
            )
...
class Profile:
    ...
    def __unpack_json__(self, _json):
        ...
        self.updated_at: datetime = datetime.fromisoformat(_json.get("updated_at"))
...
class PlayerData:
    def __init__(self, _json):
        ...
        self.summary_graph_data: SummaryGraphData = SummaryGraphData(
            _json.get("summary_graph_data")
        )
        ...
        self.general_records: GeneralRecords = GeneralRecords(
            _json.get("general_records")
        )
        self.current_season_records: CurrentSeasonRecords = CurrentSeasonRecords(
            _json.get("current_season_records")
        )
        self.past_season_ranked_records: PastSeasonRankedRecords = (
            PastSeasonRankedRecords(_json.get("past_season_ranked_records"))
        )
        ...
        if _json.get("last_played_at") is not None:
            self.last_played_at: datetime = datetime.fromisoformat(
                _json.get("last_played_at")
            )
        else:
